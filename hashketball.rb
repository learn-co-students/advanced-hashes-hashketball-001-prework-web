...
def game_hash 
  ...
  def player_numbers(team_name)
    ...
    if team_name == away_team[:team_name]
      away_players.each do |player_name, hash_of_stats|
        charlotte_numbers << hash_of_stats[:number]
        end
      return charlotte_numbers
    else
      home_players.each do |player_name, hash_of_stats|
        brooklyn_numbers << hash_of_stats[:number]
        end
  ...
  def big_shoe_rebounds
    ...
    sorted = all_players.sort_by { |name, stats| stats[:shoe]}
    # binding.pry
    a = sorted.flatten
    b = sorted.first
    c = b[1]
