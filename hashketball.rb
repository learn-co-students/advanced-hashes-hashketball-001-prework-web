...
def team_names
  game_hash.collect {|key, val| val[:team_name]}
end
...
def player_stats(player)
  game_hash.each do |location, team_data|
    return team_data[:players][player] if
team_data[:players].has_key?(player)
end
end
...
def big_shoe_rebounds
  ...
  game_hash.each do |location, team_data|
    team_data[:players].each do |player, stats|
      if stats[:shoe] > combo[0]
        combo = [stats[:shoe], stats[:rebounds]]
      end
    end
end
combo[1]
