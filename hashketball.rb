...
def player_numbers(team)
	game_hash.each{|location, team_data|
        if team_data.has_value?(team)
         return team_data[:players].collect{|name, attribute|
                   team_data[:players][name][:number]
        	    }
        end
	}
end
...
def big_shoe_rebounds  
   ...
   game_hash.each{|location, team_data|
        team_data[:players].each{|player, stats|
             if stats[:shoe] > max_shoe_rebounds[0]
             	max_shoe_rebounds = [stats[:shoe], stats[:rebounds]]
             end
        }
   }
   max_shoe_rebounds[1]
