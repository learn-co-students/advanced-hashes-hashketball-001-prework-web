...
def num_points_scored(player_input)
  ...
  data.each do |info, moredata|
    if info == :players
      moredata.each do |name, evenmoredata|
      if name == player_input
          return evenmoredata[:points]
      end
    end
    end
  end
 ...
 def player_numbers(team_name)
   ...
   game_hash.each do |side, data|
     if data[:team_name] == team_name
       data.each do |info, moredata|
         if info == :players
           moredata.each do |player, evenmoredata|
             x << evenmoredata[:number]
           end
         end
       end
     end
...
game_hash.each do |side, data|
  data[:players].each do |player, evenmoredata|
      if evenmoredata[:shoe] > shoe
        shoe = evenmoredata[:shoe]
        rebounds = evenmoredata[:rebounds]
      end
    end
  end
  rebounds
