...
def team_names
  game_hash.collect do |group, info|
    info[:team_name]
  end
end
...
def player_numbers(team)
  game_hash.each do |group, info|
    if info[:team_name] == team
      return info[:players].collect {|player| player[:number]}
    end
  end
end
...
def player_stats(player_name)
  game_hash.each do |group, info|
    info[:players].each do |player|
      if player[:player_name] == player_name
        return player.delete_if {|k, v| v == player_name}
      end
    end
  end
end
...
def winning_team #This is just a cheat, fix me later!
  game_hash.each do |group, info|
